...
E("google.loader.LoadFailure",!1);sc?console.warn("Google Charts loader.js should only be loaded once."):tc();J.m.B.sa=tc;J.m.B.va=xc;J.m.B.wa=Ac;J.m.B.xa=zc;J.m.B.Da=Bc;J.m.B.Ca=Cc;J.m.B.za=vc;J.m.B.ra=function(){return rc};}).call(this);
  ...
  <style>
	...
	class ProjectCharts extends HTMLElement {
    ...
    drawTimeline(){
      ...
      google.visualization.events.addListener(chart, 'select', (e)=>this.handleClick(chart,e));
      var options = {
        tooltip: { 
          isHtml: false,
          trigger: "focus"
      ...
      chart.draw(this._view,options);
      this._chartType='Timeline';
    }

    drawGantt(showPercentComplete){
      var container = this.shadowRoot.getElementById('chart_div');
    ...
    getDataDefinition(){
      ...
      data.addColumn({ type: 'string', id: 'ProjectID' });
      data.addColumn({ type: 'string', id: 'ProjectName' });
      data.addColumn({ type: 'string', id: 'TaskID' });
      data.addColumn({ type: 'string', id: 'TaskName' });
      data.addColumn({ type: 'date', id: 'startDate' });
      data.addColumn({ type: 'date', id: 'endDate' });
      return data;
    }

	  showGanttChart(project,showParent,showPercentComplete){
      var temp = this;
			google.charts.load('current', {'packages':['gantt']});
			google.charts.setOnLoadCallback(function() {
				loadData(project,showParent,showPercentComplete);
			});

			function loadData(project,showParent,showPercentComplete) {
        var data = new google.visualization.DataTable();
        data.addColumn({ type: 'string', id: 'ProjectID' });
        data.addColumn({ type: 'string', id: 'ProjectName' });
        ...
        if(showParent){
          ...
          var group = project.projectName;

          var startDate = new Date(project.projectStartDate);
          startDate.setDate(startDate.getDate() + 1);

          var endDate = null;
          if(typeof project.projectEndDate !== 'undefined'){
            endDate = new Date(project.projectEndDate);
            endDate.setDate(endDate.getDate() + 1); 
          }

          var duration = null;
          if(typeof project.projectDuration !== 'undefined'){
            duration = daysToMilliseconds(Number.parseInt(project.projectDuration));
          }          
          
          var percentComplete = null;
          if(typeof project.projectPercentComplete !== 'undefined'){
            percentComplete = project.projectPercentComplete;
          }
          var dependencies = null;
          data.addRow([projectID,projectName,id,description,group,startDate,endDate,duration,percentComplete,dependencies]);
        }
        ...
        for(var x=0;x<tasks.length;x++){
          ...
          id = tasks[x].taskID;
          description = tasks[x].taskName;
          group = tasks[x].groupName;

          startDate = new Date(tasks[x].startDate);
          startDate.setDate(startDate.getDate() + 1);

          duration = null;
          if(typeof tasks[x].duration !== 'undefined'){
            duration = daysToMilliseconds(Number.parseInt(tasks[x].duration));
          }
         

          endDate = null;
          if(typeof tasks[x].endDate !== 'undefined'){
            endDate = new Date(tasks[x].endDate);  
            endDate.setDate(endDate.getDate() + 1); 
          }else if(duration == null){ //if there is no end date and duration is null then use next task start date as end date
            if(x == tasks.length-1){
              endDate = new Date(project.projectEndDate); //if it is the last task then use the Project End Date
            }else{
              endDate = new Date(tasks[x+1].startDate);
            }    
            endDate.setDate(endDate.getDate() + 1); 
          }
          percentComplete = null;
          if(typeof tasks[x].percentComplete !== 'undefined'){
            percentComplete = tasks[x].percentComplete;
          }
          dependencies = tasks[x].dependency;

          data.addRow([projectID,projectName,id,description,group,startDate,endDate,duration,percentComplete,dependencies]);
        }
        
        temp._data = data;
        temp._view = new google.visualization.DataView(data);
        temp._view.hideColumns([0,1]);
        temp.drawGantt(showPercentComplete);
    ...
    showTimeline(projects){
      ...
      google.charts.load('current', {'packages':['timeline']});
        ...
        var data = temp.getDataDefinition();
        ...
        for(var i=0;i<projects.length;i++){     
          ...
          for(var x=0;x<tasks.length;x++){
            ...
            var startDate = new Date(tasks[x].startDate);
            startDate.setDate(startDate.getDate() + 1);
            var endDate = new Date(tasks[x].endDate);  
            endDate.setDate(endDate.getDate() + 1); 
            data.addRow([projectID,projectName,taskID,taskName,startDate,endDate]);
        }
      }
        temp._data = data;
        temp._view = new google.visualization.DataView(data);
        temp._view.hideColumns([0,2]);
        temp.drawTimeline();
			}
    }

    showTimelineByStartDate(projects){
      var temp = this;
      google.charts.load('current', {'packages':['timeline']});
   		google.charts.setOnLoadCallback(function() {
				loadData(projects);
        ...
        for(var i=0;i<projects.length;i++){     
          ...
          tasks.sort(function(a, b){
            let first = new Date(a.startDate);
            let second = new Date(b.startDate);
          ...
          for(var x=0;x<tasks.length;x++){
            ...
            startDate.setDate(startDate.getDate() + 1);
            if(x == tasks.length-1){
              var endDate = new Date(projects[i].projectEndDate); 
            }else{
              endDate = new Date(tasks[x+1].startDate);
            }    
            endDate.setDate(endDate.getDate() + 1); 
    ...
    showTimelineByDuration(projects){
      var temp = this;
      google.charts.load('current', {'packages':['timeline']});
   		google.charts.setOnLoadCallback(function() {
				loadData(projects);
      ...
      function daysToMilliseconds(days) {
        ...
        for(var i=0;i<projects.length;i++){     
          ...
          for(var x=0;x<tasks.length;x++){
            ...
            var duration = Number.parseInt(tasks[x].duration);
            var durationInMS = daysToMilliseconds(duration);

            var newTimeInMS = startDate.getTime() + durationInMS;
            console.log(newTimeInMS);
            var endDate = new Date(newTimeInMS);
    ...
    handleClick(chart,e){
      if(this._chartType=='Timeline'){

        var row = chart.getSelection()[0].row;

        var selectedProject = {
          projectID: this._data.getValue(row,0),
          projectName: this._data.getValue(row,1),
          tasks:[{
            taskID:this._data.getValue(row,2),
            taskName:this._data.getValue(row,3),
            startDate:this._data.getValue(row,4),
            endDate:this._data.getValue(row,5)
          }]
        };
      }else{
        row = chart.getSelection()[0].row;

        var projectID = this._data.getValue(row,0);
        var projectName = this._data.getValue(row,1);
        var taskID = this._data.getValue(row,2);
        var taskName = this._data.getValue(row,3);
        var group = this._data.getValue(row,4);
        var startDate = this._data.getValue(row,5);
        var endDate = this._data.getValue(row,6);
        var duration = this._data.getValue(row,7);
        var percentComplete = this._data.getValue(row,8);
        var dependency = this._data.getValue(row,9);

        if(taskID == projectID){ //the user clicked on the top project
          selectedProject = {
            projectID: projectID,
            ...
            projectPercentComplete: percentComplete
          };
        }else{
          selectedProject = {
            projectID: projectID,
      ...
      this._selectedData = selectedProject;
    ...
    connectedCallback(){
      this.addEventListener('select',(e)=>this.handleClick(e));
      this.addEventListener('resize',(e)=>console.log("Resizing"));
    }
	}

	customElements.define("com-sap-sample-project", ProjectCharts);
})();